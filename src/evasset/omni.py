...
_IN_ASSET_SAFETY = f"""a.item_id IN (
        WITH RECURSIVE safety(item_id) AS (
            SELECT item_id FROM assets WHERE location_flag = 'AssetSafety'
            UNION
            SELECT a2.item_id FROM assets a2 JOIN safety s ON a2.location_id = s.item_id
        )
        SELECT item_id FROM safety
    ) OR a.root_location_id = {ASSET_SAFETY_LOCATION_ID}"""
